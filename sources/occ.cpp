...
void* decodeSymbolFromCroppedImg(void * data) {
    struct threadData *thData = (struct threadData *) data;
    Mat img = thData->lamp;
    roiData *roi = thData->roi;
    uint8_t rxMsg = 0;
    ...
    */
    ...
    if(roi->sfdDecodeFlag)
    {
        ...
        decMsgs[thData->tId] = 0;
        //decMsgs[thData->tId] = std::bitset<8>(0).to_string();
    ...
    if(index == 11)
    {
        ...
        if(roi->noOfBits == MAX_PAYLOAD){
            ...
            pthread_mutex_lock(&msgMutex);
            decMsgs[thData->tId] = rxMsg;
            //decMsgs[thData->tId] = std::bitset<8>(rxMsg).to_string();
            pthread_mutex_unlock(&msgMutex);
...
//                                    cv::Size(10, 10));
        ...
        for(int i = 0;i < bboxes.size(); i++){
            ...
            if(pthread_mutex_init(&msgMutex, NULL)!= 0){
    ...
    }else
    {
        ...
        int noOfLamps = multiTracker->getObjects().size();
        pthread_t threads[noOfLamps];
        pthread_attr_t attr;
        pthread_attr_init(&attr);
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
        int status;
        struct threadData thData[noOfLamps];
        for (int i = 0; i < noOfLamps; i++)
        {
            ...
            thData[i].tId = i;
            thData[i].lamp = croppedRef;
            thData[i].roi = &sources[i];
            status = pthread_create(&threads[i],&attr, decodeSymbolFromCroppedImg,(void *)&thData[i]);
            if(status){
                cout << "Unable to create threads" << endl;
                exit(-1);
            }
            //uint8_t decMsg = decodeSymbolFromCroppedImg(croppedRef,&sources[i]);
            //decoded_data.msg.push_back(std::bitset<8>(decMsg).to_string());
        }
        ...
        for(int i=0; i< noOfLamps; i++)
            decoded_data.msg.push_back(decMsgs[i]);


        //for(auto msg : decoded_data.msg){
            //if(msg!=0)
               // cout << std::bitset<8>(msg)<< "\t";
        //}
        //if(decoded_data.msg.size() == 2)
         //cout << bitset<8>(decoded_data.msg[0]) << "\t" << bitset<8>(decoded_data.msg[1]) << endl;
